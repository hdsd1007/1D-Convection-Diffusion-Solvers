// Solving 1D Steady State Diffusion Equation using C++
// - A script to set up and solve the 1D diffusion equation for conduction in a bar
// [Chapter 2]
...
void Diffusion1D(double barLength, int nCells, double area, double cond, double tempLeft, double tempRight, double heatSourcePerVol, bool printSetup, bool printSolution, bool printOutput)
{
    std::cout << "------------------------------------------------" << std::endl;
    ...
    Su[0] = Su[0] + 2*tempLeft*DA[0];
    Su[nCells-1] = Su[nCells-1] + 2*tempRight*DA[nCells];

    //aL Coefficients
    std::vector<double> aL(DA.begin(),DA.begin() + nCells);
    aL[0] = 0.0;

    //aR Coefficients
    std::vector<double> aR(DA.begin() + 1, DA.end());
    aR[nCells-1] = 0.0;
...
int main(int argc,char* argv[])
{
    ...
    Diffusion1D(barLength, nCells, area, cond, tempLeft, tempRight,  heatSourcePerVol, printSetup, printSolution, printOutput);
